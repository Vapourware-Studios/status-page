...
No server to babysit. No database to host. Globally distributed and effectively free.
...
Statch is a self-hosted status page for people who would rather commit a YAML file than click around in a dashboard. Define your monitors, groups, branding, and alert channels in [`status.config.yml`](./status.config.yml); deploy; done. Everything else — incidents, maintenance, and subscribers — is handled from the admin panel.

**Stack:** Hono · D1 + Drizzle ORM · React + Vite + Tailwind v4 · Cloudflare Cron

## Why Statch

* **Runs on one Worker.** Uptime Kuma needs an always-on box; Statch runs on the Cloudflare edge for ~$0 and never sleeps.
...
* **Auto-discovering agents.** Enroll a machine the same way you would run a Cloudflare Tunnel: copy a one-liner, paste it on the box, and it shows up. No pairing codes.
* **Rich monitoring.** HTTP/TCP/push/keyword checks, response-time **degraded** states, status-code and body assertions, TLS-failure detection, dead-man’s-switch heartbeats, and N-failure confirmation to prevent flapping.
* **Alerts everywhere.** Discord rich embeds, Slack, Telegram, Microsoft Teams, PagerDuty, generic webhooks, Web Push, and an RSS feed.
* **“It’s not you, it’s Cloudflare.”** When a monitor goes down, Statch checks Cloudflare’s own status page and, if the orange cloud is on fire, says so on the incident.

## Quick start

### One-click

Click the **Deploy to Cloudflare** button above. It forks the repo, creates the `statch-db` D1 database defined in [`wrangler.jsonc`](./wrangler.jsonc), applies migrations, and deploys the Worker. Then set your secrets below.
...
npx wrangler secret put SESSION_SECRET     # any random 32+ character string

# 4. Edit status.config.yml to taste, then ship it
npm run deploy
```

Local dev: `npm run dev` using Vite + Miniflare with local D1 → http://localhost:5173.
...
## Add a server to monitor using auto-discovery
...
Run it on the machine. It enrolls itself, starts heartbeating, and appears live. No pairing code needs to be typed back in. Uninstall it with `statch-agent --uninstall`.
...
| POST   | `/api/agents/enroll`               | enrollment token | Auto-discover a push monitor      |
| POST   | `/api/heartbeat`                   | agent token      | Push monitor heartbeat            |
| POST   | `/api/admin/monitors/enroll-token` | cookie           | Mint an enrollment one-liner      |
...
You are free to use, study, share, and modify Statch. If you distribute a modified version, it must also be GPLv3 and its source must be made available. It comes with no warranty.
